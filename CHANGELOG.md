...
- First-time visitors now see a small gallery of fully styled example monograms — gradients, filled Frames with a cut-out, gradient Backgrounds — right after typing their initials, before the full Design list. Pick one to start from it, or "See all designs instead" to land exactly where onboarding used to. The editor also shows a one-time dismissible note pointing out the Design/Frame/Colors/Export tabs the first time you arrive.
