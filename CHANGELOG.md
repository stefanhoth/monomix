...
- A new **Easy/Full mode** toggle, always visible above the sidebar: Easy shows a small gallery of fully styled example monograms plus a handful of curated color combinations and Export — nothing else — while Full is today's complete set of controls (Frame, gradients, opacity, background image). First-time visitors now land straight in the real editor in Easy mode instead of a separate one-time gallery screen, and can switch to Full at any time; the choice is remembered for next time. The sidebar's four sections (Design, Frame, Colors, Export) are now shown as a connected, numbered sequence instead of flat tabs, and the app has a new considered accent color used consistently for the active step and primary actions.
- Background images can now be zoomed and repositioned instead of always sitting full-bleed and centered: drag the preview to move the picture, or use the Zoom/Horizontal/Vertical sliders in the Colors tab. Panning is bounded, so the image can never be dragged far enough to leave a gap. Included in every export format.
