...
- **A shared monogram travels entirely inside the URL hash, and a background image is explicitly dropped from it rather than truncated (issue #121).** MonoMix has no server and no accounts (ADR 0002, CONTEXT.md), so there is nothing to look a share id up in — the link either carries the monogram or the feature doesn't exist. `src/lib/share-link.ts` encodes `ProjectSettings` as short-keyed JSON → URL-safe base64 under a `#m=<payload>` hash, reusing the `history.replaceState`-only approach `AboutPanel`'s `#about` hash already established (2026-07-16 entry below) so opening or following a link never grows browser history. Every field matching `DEFAULT_PROJECT_SETTINGS` is omitted and restored from the same defaults on the other side, which keeps a typical link under ~150 characters instead of ~350; the payload carries a `v` version marker and an unrecognized version is **rejected outright** rather than best-effort parsed, since half-decoding someone else's monogram into a silently-wrong one is worse than saying the link couldn't be read.
  ...
  - **Decoding delegates field validation to `normalizeProject` rather than re-implementing it**, since a share payload is exactly as untrusted as a record read back from IndexedDB and that function is already the one defensive "untyped record in, valid Project out" boundary. The one thing it _doesn't_ cover is added on top: letters go through `sanitizeLettersInput` too, because a link is the only way letters reach the editor without passing through the letters field and nothing downstream re-enforces the 1–3 A–Z cap (the engine would happily lay out 50 glyphs). No XSS surface beyond that — letters become glyph paths, never interpolated markup, and colors still pass `sanitizeColor`.
