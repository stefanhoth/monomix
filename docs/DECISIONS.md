...
- **Letters and Frames gained Gradient fills by generalizing `background.ts`'s gradient machinery into a `Paint` abstraction, not by duplicating it (issue #122).** `src/engine/paint.ts` now owns the `Gradient` types, `sanitizeStops`, the content-hashed `<defs>` builder, and a new `Paint = string | Gradient` union; `background.ts` consumes it like any other caller rather than remaining the only thing in the app that could carry a gradient. `resolvePaint(paint, fallback, idScope)` returns `{ defs, value }` — `defs` is `""` for a solid color, so every solid-painted monogram's SVG stays **byte-identical** to what shipped before (the same "no-op attributes never bloat the output" rule `lettersOpacity` follows). `ComposeOptions.lettersColor` and `FrameOptions.color`/`fill` all widen from `string` to `Paint`; `composeFrame` keeps its `string` in / `string` out signature and simply prepends its own `<defs>`, which is valid SVG anywhere and avoided a churny return-type change across its existing callers and tests.
  ...
  - **Gallery tiles paint with one representative solid (`paintSolidColor` — the gradient's first stop), never the gradient itself.** This is the feature's real hazard, and it is the _same_ one issue #65's `<mask>` ids hit (2026-07-17 entry below): a `<defs id>` is document-global, and the Design/Frame gallery panels stay **mounted but hidden** (`hidden` → `display:none`) when their tab isn't active, because "tabs are views, not gates". Letting ~30 tiles each emit the live preview's own content-hashed gradient id would reproduce that corruption exactly. Background gradients never hit this only because the two galleries don't take a `background` option at all. Tiles preview a Design's shape and a Frame's silhouette, and a thumbnail is not where a gradient reads anyway. `letters-frame-gradient.spec.ts` asserts no two gradient defs share an id across the whole page, mirroring `frame-fill.spec.ts`'s existing `<mask>` guard. `NewProjectSurface`'s remix thumbnails _do_ render real gradients — they mount only while the New surface is open (`{#if open}`), so they're never hidden-but-present, exactly as background gradients already were.
