...
- **Font credits page** — ADR 0003 promises "an about page credits every font"; no credits UI exists yet, and the catalog is now 23 fonts deep. Surface every family + license (data already lives in `src/engine/fonts.ts`).
- **Dialog focus management** — `WhatsNewPanel` and `NewProjectSurface` set `role="dialog" aria-modal="true"` but neither moves focus into the dialog nor traps it; keyboard users can tab into the obscured page behind. Fix both together (shared action or small dialog wrapper).
