...
Five things follow from that. This repo has been bitten by the first four; the fifth is what they don't cover.
...
5. **What the string assumes of its renderer.** The four rules above are about what the engine *emits*; this one is about what it takes for granted. Geometry outside the viewBox, reliance on the root `<svg>` to clip, references to anything not embedded, assumptions about font availability — each is a contract with the consumer, and this repo has four of them (inline preview, canvas raster, PDF via svg2pdf, and the raw downloaded `.svg`, which is the one that gets opened in a vector editor rather than a browser). When you find such an assumption, walk all four and say which hold and which don't.
